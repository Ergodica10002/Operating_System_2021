...
#define PSJF 4
// ALG can be 
// DEFAULT FCFS RR SJF PSJF
#define ALG RR

#if ALG == RR
static int RR_tick_passed = 0;
#endif
...
void schedule(void){
...
#if ALG == SJF
        struct thread* SJ_thread = current_thread;
        struct thread* search_thread = current_thread->next;
        while (search_thread != current_thread){
            if (search_thread->remain_execution_time < SJ_thread->remain_execution_time){
                SJ_thread = search_thread;
            } else if (search_thread->remain_execution_time == SJ_thread->remain_execution_time){
                if (search_thread->ID < SJ_thread->ID){
                    SJ_thread = search_thread;
                }
            }
    ...
    else
    {
        ...
        if (current_thread->is_exited == 1 || current_thread->is_yield == 1){
            struct thread* SJ_thread = current_thread;
            while (SJ_thread->is_exited == 1){
                SJ_thread = SJ_thread->next;
                if (SJ_thread == current_thread){
                    break;
                }
            }
            struct thread* search_thread = current_thread->next;
